...
$toRow = ($page * $perPage);
$sqlQuery = "SELECT UI,person.Name as pName,Field7,Field9,Field10,Field2,Field27,personID FROM person  left join CV_Places on (CV_Places.ID = person.Field27) where Field27!='0' and Field27 IS NOT NULL  and `online` = '1' ".$cQuery." ORDER BY Field2 ASC LIMIT $startFrom, $perPage";
...
$perPage = 10;
$stmt = $conn->prepare("SELECT count(*) FROM person left join CV_Places on (CV_Places.ID = person.Field27) where `online` = '1' " . $cQuery);
$stmt->execute();
